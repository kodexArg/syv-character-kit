...
- **Open Questions con número de versión**: las OQs activas viven en la página Notion del proyecto (sin sufijo de versión). Cuando una se resuelve, se elimina la entrada o se reescribe la sección afectada. No se marca "OQ-12 resuelta en v0.5.0".
...
Mismo principio que el PRD: rolling release, sin versionado ni changelog. Todos los archivos bajo `docs/` (`hoja-modelo.{md,yaml}`, `tag-modelo.{md,yaml}`, `tag-modelo-ejemplos.yaml`, `tag-requeridos-por-categoria.md`, `atributos-y-efectos.md`, `user-stories.md`, y cualquier doc nuevo) describen el estado vigente. Cualquier cambio se aplica como reescritura del párrafo o sección afectados; no se anota "antes era X".

**Excepción tolerable**: open questions activas y backlog explícito viven en la página Notion del proyecto. Las que son específicas del sistema de tags pueden quedar en `tag-modelo.md §8`. No es historia — es backlog.
...
- **Tags**: notación punto `<categoria>[.<subcategoria>].<slug>`. Ver [[tag-modelo#§2 — Notación punto|docs/tag-modelo.md §2]].
