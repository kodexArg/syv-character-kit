...
- **Lore canon**: la documentación del universo *Subordinación y Valor* vive en `/Dev/SyV/syv/src/content/docs/` — **única fuente válida** (estructura: `0_proyecto/`, `1_trasfondo/`, `2_atlas/`, `3_personajes/`, `4_diegesis/`, `5_aventuras/`, `6_media/`). Para facilitar el acceso en desarrollo, existe un enlace simbólico en la raíz del proyecto como `syv_docs/` (excluido correctamente en `.gitignore`) para referencias rápidas sobre el lore. Esta carpeta es de **solo lectura** ya que corresponde a otro repositorio; cualquier modificación o escritura en los archivos de `syv_docs/` (o directamente en el repositorio `syv`) **requiere confirmación explícita del usuario**. Antes de inventar terminología (geografía, facciones, vocabulario, personajes), verificar contra esta fuente para mantener fidelidad. Otros directorios del ecosistema SyV (`/Dev/SyV/syv-obsidian/`, `/Dev/SyV/syv-battle-game-system/`) **no son canónicos para este kit**. Las decisiones de mecánica, atributos por rango, equipamiento y motor de batalla son **soberanía del kit** — se documentan acá sin remitirse a otro repo.
